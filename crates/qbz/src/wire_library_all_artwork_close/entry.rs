...
pub(crate) fn wire_library_all_artwork_close(window: &AppWindow, app_runtime: &Arc<AppRuntime<SlintAdapter>>, tokio_rt: &tokio::runtime::Runtime, image_cache: &artwork::ImageCache, settings_ctx: &Arc<settings::SettingsCtx>) -> Result<(), Box<dyn std::error::Error>> {
