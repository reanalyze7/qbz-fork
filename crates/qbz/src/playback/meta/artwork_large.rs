//! The higher-res now-playing cover decode.
//!
//! Split out of `artwork.rs`: this one decode carries the immersive-view
//! ambient background/glow derivation as well as the hover-preview art, so
//! it is far larger than the bar-size sibling and shares nothing with it
//! beyond the `ArtworkRef` funnel.
