...
//! refs never touches the Qobuz API at all: local rows attach to a
//! Qobuz playlist through the `playlist_local_tracks` sidecar table (see
//! `main.rs`'s `on_pick` local-mode branch), so both the has-check and the
//! removal below stay fully local (DB only).

/// True when EVERY pending local-mode ref is already attached to the QOBUZ
/// playlist `pid` via the local sidecar table. Blocking (DB).
pub fn already_has_refs_blocking(pid: u64, refs: &[String]) -> bool {
    if refs.is_empty() {
        return false;
    }
    let local_ids: Vec<i64> = crate::library_db::with_db(|db| {
        let locals = db.get_playlist_local_tracks(pid).unwrap_or_default();
        Ok(locals.into_iter().map(|t| t.id).collect())
    })
    .unwrap_or_default();
    refs.iter().all(|r| {
...
/// sidecar table. Blocking (DB). Returns the removed count.
